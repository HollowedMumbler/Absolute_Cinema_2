...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const [user, setUser] = useState<GameContextType["user"] | null>(null);
  ...
  const [leaderboard, setLeaderboard] = useState<
    { name: string; points: number; rank: number; avatar: string }[]
  >([]);
    ...
    setStats((prev) => {
      ...
      const newStats = {
        ...
        xpToNextLevel: levelUp
          ? Math.floor(prev.xpToNextLevel * 1.2)
          : prev.xpToNextLevel,
      };
  ...
  const logCommute = useCallback(
    async (commute: Omit<CommuteLog, "id" | "points">) => {
      const vehicle = vehicles.find((v) => v.id === commute.mode);
      const points = Math.floor(
        commute.distance * (vehicle?.ecoFactor || 1) * 10,
      );
  ...
  return (
    <GameContext.Provider
      value={{
        user: user as GameContextType["user"],
        stats,
        selectedVehicle,
        vehicles,
        badges,
